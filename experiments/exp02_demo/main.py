#!/usr/bin/env python
"""
Experiment: exp02_demo
System:     Perplexity Sonar Deep Research (демо без torch)
Pipeline:
  [SurveyBench] step 1: generation → step 2: SWR/CWR evaluation
  [SurGE]       step 3: generation → step 4: Structure_Quality / Logic
                (SH-Recall исключён — требует FlagEmbedding/torch)

Группы метрик (флаги):
  --bench-metrics   генерация по SurveyBench + оценка SWR/CWR
  --surge-metrics   генерация по SurGE      + оценка Structure_Quality/Logic
...
def parse_args():
    ...
    p.add_argument("--surge-metrics", action="store_true",
                   help="Запустить SurGE: генерация + Structure_Quality/Logic")
    args = p.parse_args()
...
def main() -> None:
    ...
    if args.surge_metrics:
        ...
        surge_eval   = ",".join(surge_cfg.get("eval_list",  ["structure_quality", "logic"]))
