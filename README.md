...
│   ├── scisage/                 #   SciSage (multi-stage survey pipeline)
...
│   ├── SciSage/                 #   используется models/scisage
...
| `scisage` | SciSage | multi-stage outline/retrieval/writing | OpenRouter + Serper |
| `autosurvey` | AutoSurvey | конвертация baseline | — |
...
| **C.2 `M_comp`** | доля явных сравнений |
...
git clone https://github.com/FlagOpen/SciSage.git  repos/SciSage
...
| Переменная | Описание |
|------------|----------|
| `OPENROUTER_API_KEY` | OpenRouter — для perplexity_dr, surveyforge, scisage |
| `OPENAI_API_KEY` | OpenAI — для surveygen_i (прямой API) |
| `GOOGLE_SERPER_KEY` | Google Serper — academic retrieval в SciSage |
...
- модели: `perplexity_dr`, `surveygen_i`, `scisage` (для последних двух ещё нужны
  `pip install -r models/surveygen_i/requirements.txt` / `make setup-scisage`)
...
make generate MODEL=scisage       DATASET=SurGE
...
Для SciSage перед bare-metal запуском:

```bash
make setup-scisage
make generate MODEL=scisage DATASET=SurGE DOCKER=0 LIMIT=0
```

SciSage пишет сырой upstream-state в `tmp/scisage/<sid>/`, а нормализованный
результат — в обычный `results/generations/SurGE_scisage/<sid>.json`.
